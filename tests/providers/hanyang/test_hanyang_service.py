...
class FakeHanyangClient:
    ...
    find_courses_calls: list[tuple[str, str, str, str, str, str]] = field(
        default_factory=list
    )
    ...
    def find_courses(
        ...
        *,
        ...
        self.find_courses_calls.append((year, semester, org_code, pgm_id, menu_id, tk))
        return {
            "DS_SUUPGS03TTM01": [
                {
                    "list": [
...
def test_hanyang_service_reads_colleges_from_program_payload() -> None:
    ...
    service = HanyangService(client=client)
